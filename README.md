...
изменять. Архитектура - это комплекс мер и договоренностей, которые отвечает на вопрос, что будет, а
не как что-то реализовать. Например, при постройке дома мы видим по чертежам и понимает что будет
какие комнаты и что в эти комнаты будет, но не отвечает какой цвет будет комната и какая модель
мебелей буде там.
...
# Event loop и Isolate

## Основа

**Isolate** - это поток (thread) с собственной областью памяти. Изоляты знают только про свои
дочерние изоляты, которые были созданы в них, но облась памяти у них все равно разные.

**Event loop** - это цикл, который создается и запускается в изоляете, и выполняет событии(event) и
микротаски(micro tasks).<br>

Event loop в приоритете ставит:

- синхронный код;
- микротаски;
- событии;

В Flutter runApp() - это функция, которая под капотом создает и запускает event loop для приложения.
Весь Flutter приложения запускается в одном изоляте с одним event loop. Почти все взаимодействия с
UI (нажатии, скролл, анимации и тд.) - это событии, которые добавляются в очередь.

То есть изолят сначала выполняет весь сихронный код в главной функции main(), и в функциях, которые
вызываются в главной функции. А потом выполняет runApp(), который запускает бесконечный event loop.

Нюансы:

1) сихронный код и микротаски блокирует UI, так как нажатии, скролл, и анимация добавляются в
   очередь событии, которые выполняется только после сихронного кода и микротасков.
2) .then() у Future это сихронный код, который вызывается сразу после завершения Future. То есть
   event loop полностью завершает выплнения Future c .then().
3) При Future.delayed event добавялется в очередь событии, только после исчетечние времени.

## Функция работает до завершения

Любая функция в Dart работает до своего завершения, мы можем преостновить его только завершив работу
приложения или убив изолят, в котором запущенна функция:

- exit({code_number});
- Isolate.current.kill();

Или как вариант в функции добавлить флаг, если оно true, то делаем return.

## Isolate

У каждого изолята:

- свой точка запуска (типо main);
- своя область памяти;
- своя event loop;
- свои очереди событий и микротасков;

Изоляты обычно нужно использовать:

- тяжелых синхронных операции
- длительное выполнения функции, которая работает в фоне (отдельном изоляте). Например, постоянное прослушка какого
  вебсокета и нужно что-то делать во время работы приложения.
- тяжелые или длительные операции, которые можно прервать в желанию

## Isolate.spawn

Isolate.spawn - это метод, который создает изолят.<br>

Параметры функции spawn:

- entrypoint (точка входа в изолят, функция). Функция должна быть глобальной или статическим методом
- message - аргумент, который передает в изолят
- onExit - простой тип данных (int, string, null), который вернется в главной изолят после закрытия
  созданного
- onError - тоже самое как и onExit, только при ошибке

## SendPort и ReceivePort

Когда изоляты общаются между собой, то данные превращаются сначало в бинарные данные, а потом
отправляются. А другой изолят его конвертирует опять в данные в Dart.<br>

Есть порты для передача и прослушки данных между изолятами:

- SendPort - это как sink в BLoC для отправки событи<br>
- ReceivePort - это для получие emit() в блоке

Под капотом это просто Stream через которые проходят данные. Эти порты можно использовать для
передач данных между несколькими изолятами.

Между изолятами можно передавми любые объекты, даже свои классы, но желательно простые типы данных
для производительности. Например, двух сторонняя коммуникация между двумя изолятами:
![img_4.png](img_4.png)

## Сколько изолятов можно создавать

Кол-во новых изолятов, кроме основного, не должно быть больше чем кол-во просецеров дейвайса:

```dart
Platform.numberOfProcessors - 1;
```

Минус один так как у нас еще есть основной изолят.
